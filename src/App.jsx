...
export default function App() {
  ...
  const [gamesData, setGamesData] = useState([]); // To store full game data including ids
  ...
  // Helper function to generate Steam store URL
  const generateSteamUrl = (appId) => {
  ...
  const fetchAndSpin = async () => {
    if (mode === "custom") {
      ...
      setGames(selected);
      setGamesData(selected.map(item => ({ name: item, id: null }))); // For custom lists, no Steam IDs
      setShouldSpin(true);
      return;
    }
    ...
    try {
      ...
      const selectedGames = Array.from({ length: totalGames }, () => {
        return filtered[Math.floor(Math.random() * filtered.length)];
      });

      setGamesData(selectedGames);
      setGames(selectedGames.map(game => game.name));
      setShouldSpin(true);
    } catch (err) {
  ...
  const finishSpin = () => {
    ...
    const resultData = gamesData[winnerIndex];
    setWinner(result);
  ...
  // This helps debug what's happening with the game data
    ...
    <div style={{ padding: "30px", color: "#fff", fontFamily: "sans-serif" }}>
      ...
      >
        <div ref={containerRef} style={{ willChange: "transform" }}>
          {[...gamesData, ...gamesData].map((gameData, i) => {
            const isWinner = (i % games.length) === finalWinnerIndex;
            const isSteamGame = mode === "steam" && gameData && gameData.id;
            const realIndex = i % games.length;
            ...
            return (
              <div
                ...
                style={{
                  ...
                  zIndex: 5, // Ensure links are clickable
                }}
              >
                {isSteamGame ? (
                  <a 
                    href={generateSteamUrl(gameData.id)} 
                    target="_blank"
                    rel="noopener noreferrer"
                    onClick={(e) => {
                      // If spinning, don't navigate - prevent default behavior
                      if (spinning && !isWinner) {
                        e.stopPropagation();
                        return;
                      }
                    ...
                    style={{
                      ...
                      fontSize: isWinner ? "24px" : "18px",
                      fontWeight: isWinner ? "bold" : "normal",
                      pointerEvents: spinning && !isWinner ? "none" : "auto", // Only disable during spin for non-winners
                    }}
                  >
                    {gameData.name}
                  </a>
      ...
      <div style={{ marginTop: "20px", padding: "10px", backgroundColor: "#222" }}>
        ...
        <ul style={{ listStyle: "none", padding: 0 }}>
          {history.map((entry, index) => {
            const isSteamGame = entry && entry.id; // Simplified check for Steam games in history
            ...
            return (
              ...
              >
                {isSteamGame ? (
                  <a 
                    href={generateSteamUrl(entry.id)} 
                    target="_blank"
                    rel="noopener noreferrer"
                    style={linkStyle}
                  >
